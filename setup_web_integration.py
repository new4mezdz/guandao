# deploy_visual_web.py - 部署可视化Web应用

import os
import sys


def create_visual_web_app():
    """创建保持原有可视化的Web应用"""
    print("🎨 创建可视化Web应用...")
    ...
    with open('visual_web_app.py', 'w', encoding='utf-8') as f:
        # 这里粘贴上面完整的代码
        f.write('''# visual_web_app.py - 保持原有可视化界面，调用真实算法
...
def check_requirements():
    ...
    # 检查文件
    required_files = ['isolate_leakage.py', 'my_database.db']
    missing_files = []
...
def main():
    ...
    # 创建应用
    create_visual_web_app()

    # 完成提示
    print("\n🎉 部署完成！")
    print("=" * 60)
    print("📋 启动步骤:")
    print("1. 运行: python visual_web_app.py")
    print("2. 访问: http://localhost:5000")
    print("3. 享受与test2.py完全相同的可视化界面")
    print("4. 但现在调用的是真实的隔离算法！")
    print("=" * 60)
    print("✨ 特性:")
    print("- 🎨 保持原有的漂亮D3.js可视化")
    print("- 🧠 调用真实的分层隔离算法")
    print("- 🔄 所有交互功能完全保留")
    print("- 📊 真实的最小割计算结果")
    print("=" * 60)
    ...
    # 询问是否立即启动
    choice = input("\n是否立即启动可视化Web服务器? (y/n): ").lower()
    if choice == 'y':
        print("🚀 启动中...")
        try:
            os.system("python visual_web_app.py")
        except KeyboardInterrupt:
            print("\n👋 服务器已停止")
    else:
        print("👍 稍后运行: python visual_web_app.py")
