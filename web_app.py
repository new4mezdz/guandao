...
def index():
    ...
    # 原有的HTML模板，只修改 performRealIsolation 函数
...
<body>
    ...
    <script>
        ...
        function displayIsolationResult(result, leakTarget, leakType) {{
            ...
            // 生成结果显示内容
            const targetInfo = leakType === "节点漏损" ? "节点: " + leakTarget : "管道: " + leakTarget;

            let resultHtml = 
                "<div class='result-panel'>" +
                "<strong>🔍 真实算法隔离结果</strong><br>" +
                "<strong>算法:</strong> " + (result.isolation_method || "集成隔离算法") + "<br>" +
            ...
            resultHtml += "<strong>影响管道:</strong> " + (result.affected_pipes_count || 0) + " 条<br>";

            if (result.affected_pipes && result.affected_pipes.length > 0) {{
                const displayPipes = result.affected_pipes.slice(0, 10);
                resultHtml += "<small style='color: #666;'>管道ID: " + displayPipes.join(', ');
                if (result.affected_pipes.length > 10) {{
                    resultHtml += " (显示前10个)";
                }}
                resultHtml += "</small><br>";
            }}
            ...
            if (result.priority) {{
                resultHtml += "<strong>优先级:</strong> " + result.priority + "<br>";
            }}

            if (result.emergency_notification) {{
                resultHtml += "<p style='color: red; font-weight: bold;'>🚨 紧急通知: 需要立即处理</p>";
            }}
            ...
            if (result.affected_pipes && result.affected_pipes.length > 0) {{
                ...
                result.affected_pipes.forEach(function(pipeId) {{
                    const valve = valvesData[pipeId];
                    const valveId = valve ? valve.id : "无阀门";
                    const valveStatus = valve ? valve.status : "N/A";
                    resultHtml += "<tr>";
            ...
            updateStatus(statusIcon + " 真实算法执行完成 - " + result.recommendation);
        }}
        ...
        function updateCoordinateDisplay() {{
            ...
            const dbXEl = document.getElementById("dbX");
            const dbYEl = document.getElementById("dbY");
            const d3XEl = document.getElementById("d3X");
            const d3YEl = document.getElementById("d3Y");
        ...
        function updateCurrentCoordinates() {{
            ...
            try {{
                ...
                const currentX = (selectedNode.x * transform.k + transform.x);
                const currentY = (selectedNode.y * transform.k + transform.y);

                const screenXEl = document.getElementById("screenX");
                const screenYEl = document.getElementById("screenY");
...
def isolate_api():
    ...
    try:
        ...
        print(f"✅ 算法执行结果: {result.get('success', True)}")

        # 确保返回的数据格式兼容前端
        if not result.get('success', True):  # 如果明确失败
            response = {
                "success": False,
                "error": result.get('error', '未知错误'),
                "need_close_valves": [],
                "lost_valves": result.get('lost_valves', []),
                "isolatable": False,
                "affected_pipes": [],
                "affected_pipes_count": 0,
                "cut_edges": [],
                "leak_type": data['leak_type'],
                "recommendation": result.get('recommendation', '隔离失败')
            }
        else:
            response = {
                "success": True,
                "need_close_valves": result.get('need_close_valves', []),
                "lost_valves": result.get('lost_valves', []),
                "isolatable": result.get('isolatable', True),
                "affected_pipes": result.get('affected_pipes', []),
                "affected_pipes_count": result.get('affected_pipes_count', 0),
                "cut_edges": result.get('cut_edges', []),
                "leak_type": result.get('leak_type', data['leak_type']),
                "recommendation": result.get('recommendation', '隔离完成'),
                "isolation_method": result.get('isolation_method', '集成隔离算法'),
                "priority": result.get('priority', '正常'),
                "emergency_notification": result.get('emergency_notification', False)
            }
